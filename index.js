 ...
 * TAPI JAN HAPUS THANK TO NYA OKAY
 * 
 * THX TO
 * MHANKBARBAR < BASE > 
 * ADULALHY 
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Kaa\n'
            + 'ORG: OwnerBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=994407339135:+994 40 733 91 35\n' 
            + 'END:VCARD'
prefix = '#' 
blocked = []   
limitawal = 25
...
replitx = '*WhatsApp Bot*'
ari = '*WhatsApp Bot*'

/******** OWNER NUMBER**********/
const ownerNumber = ['994407339135@s.whatsapp.net','6283504456988@s.whatsapp.net']   
const pacarNumber = ["994407339135@s.whatsapp.net"]  
/************************************/
...
//---Metadata stiker
			function addMetadata(packname, author) {	
				if (!packname) packname = 'I Hope Your Fine'; if (!author) author = 'Kaa';	
	...
	ara.on('chat-update', async (mek) => {
      ...
      for (let kemem of bad) {
		...
		case 'daftar':
                	...
                	if (umurUser > 25) return reply(`Njirr om om ga bisa daftar:v`)
                	if (umurUser < 12) return reply(`Maaf, kamu belum bisa mendaftar`)
		...
		case 'menu':       
				    ...
				    costum('[❗] WAIT A MINUTE!', text, tescuk, ari)
...
///////////////////
		...
		case 'apkpure':
				    ...
				    if (isBanned) return reply('Kamu sudah terbenned!')
				    costum('[❗] LOADING!', text, tescuk, ari)
				    data = await fetchJson(`https://api.zeks.xyz/api/apkpure?q=${body.slice(9)}&apikey=GratisApi`, {method: 'get'})
				    teks = '[☆]=================[☆]\n'
...
/////////////////////
		...
		case 'igstalk':
				    ...
				    costum('[❗] LOADING!', text, tescuk, ari)
					asw = await fetchJson(`https://api.zeks.xyz/api/igstalk?username=${F}&apikey=GratisApi`)
...
//////////////////////
        ...
        case 'tribunews':
				    ...
				    costum('[❗] LOADING!', text, tescuk, ari)
					ara.updatePresence(from, Presence.composing) 
					data = await fetchJson(`https://api.zeks.xyz/api/tribunews?apikey=GratisApi`, {method: 'get'})
					teks = '[☆]=================[☆]\n'
					for (let i of data.result) {
					teks += `❒ *JUDUL* : ${i.title}\n❒ *TIME* : ${i.time}\n❒ *LINK* : ${i.url}\n❒ *KETERANGAN* : ${i.ket}\n[☆]=================[☆]\n`
					}
        ...
        case 'foxnews': 
				    ...
				    costum('[❗] LOADING!', text, tescuk, ari)
					ara.updatePresence(from, Presence.composing) 
					data = await fetchJson(`https://api.zeks.xyz/api/foxnews?apikey=GratisApi`, {method: 'get'})
					teks = '[☆]=================[☆]\n'
					for (let i of data.result) {
					teks += `❒ *JUDUL* : ${i.title}\n❒ *LINK* : ${i.url}\n❒ *COUNTRY* : ${i.country}\n❒ *CONTENT* : ${i.content}\n❒ *TIME* : ${i.time}\n[☆]=================[☆]\n`
					}
        ...
        case 'searchfilm':
				    ...
				    costum('[❗] LOADING!', text, tescuk, ari)
                    pshh = `${body.slice(12)}`
                    anu = await fetchJson(`https://api.zeks.xyz/api/film/2?q=${pshh}&apikey=GratisApi`, {method: 'get'})
                    puree = '[☆]======================[☆]\n'
...
/////////////////////
		...
		case 'sedekah':
					...
					await reply(`Botwea BOT kasih kamu hadiah, Botwea BOT akan berikan sebanyak *${one}Xp* untuk kamu`)
        		    }else{
					const mining = Math.ceil(Math.random() * 10000)
					addLevelingXp(sender, mining)
					await reply(`*Selamat* ${pushname} kamu mendapatkan *${mining}Xp* dari Botwea Bot`)
					}
					await limitAdd(sender)
					break
        case 'hidetag':
		...
		case 'kick':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('*Tag target yang ingin BOT kick*!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
					teks = ''
					for (let _ of mentioned) {
					teks += `*Asik BOT bisa kick, lu gua kick* !! :\n`
					teks += `@_.split('@')[0]`
					}
					mentions(teks, mentioned, true)
...
/////////////////////
        ...
        case 'developer':
                    ...
                    ara.sendMessage(from, 'Tuh nomer owner ku jangan lupa di save',MessageType.text, { quoted: mek} )
					break    
        case 'admin':
				    if (!isOwner) return reply(ind.ownerb())
				    admm = body.slice(7)
...
////////////////
		...
		case 'nulis':
				    ...
				    ct = body.slice(6)
				    costum('[❗] LOADING!', text, tescuk, ari)
				    ct = await getBuffer(`https://api.zeks.xyz/api/nulis?text=${ct}&apikey=GratisApi`)
		...
		case 'tahta':
                    ...
                    if (args.length == 0) return reply(`Usage: ${prefix} + command} text\nExample: ${prefix + command} Botwea`) 
                    teks = args.join(" ") 
				    costum('[❗] LOADING!', text, tescuk, ari)
                    buffer = await getBuffer(`https://api.zeks.xyz/api/hartatahta?text=${teks}&apikey=GratisApi`) 
		...
		case 'googletext':              	
					...
					if (args.length < 1) return reply(`Masukkan kata \nContoh : ${prefix}googletext Botwea/cans/abiezz`)
				    costum('[❗] LOADING!', text, tescuk, ari)
        ...
        case 'glitchtext':					
					...
					var gli = gh.split("/")[0];
					var tch = gh.split("/")[1];
					if (args.length < 1) return reply(`Masukkan kata \nContoh : ${prefix}glitchtext Botwea/Cans`)
				    costum('[❗] LOADING!', text, tescuk, ari)
					buffer = await getBuffer(`https://api.zeks.xyz/api/gtext?text1=${gli}&text2=${tch}&apikey=GratisApi`)
...
///////////////////
		...
		case 'alay':
					...
					data = await fetchJson(`https://api.zeks.xyz/api/alaymaker?kata=${body.slice(6)}&apikey=GratisApi`)
					reply(data.result)
				    await limitAdd(sender)
					break
		case 'fml':	
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname))
			    	if (isBanned) return reply('Maaf kamu sudah terbenned!')
					data = await fetchJson(`https://tobz-api.herokuapp.com/api/randomfmylife?apikey=Tobzzz17`)
					hasil = data.result
					reply(hasil)
					await limitAdd(sender)
					break	
		...
		case 'pantun':
				    ...
				    if (isBanned) return reply('Maaf kamu sudah terbenned!')
					gatauda = body.slice(8)					
					anu = await fetchJson(`https://api.zeks.xyz/api/pantun?apikey=GratisApi`, {method: 'get'})
...
play = body.slice(7)
        ...
        case 'ytmp4':
				    ...
				    costum('[❗] LOADING!', text, tescuk, ari)
					anu = await fetchJson(`https://api.zeks.xyz/api/ytmp4?url=${body.slice(7)}&apikey=GratisApi`)
...
mimetype: 'video/mp4', filename: `${anu.result.title}.mp4`, quoted: mek
  ...
  break
		case 'play':   
				    ...
				    costum('[❗] LOADING!', text, tescuk, ari)
				    play = body.slice(5)
				    anu = await fetchJson(`https://api.zeks.xyz/api/ytplaymp3?q=${play}&apikey=GratisApi`)
...
///////////////////////
		...
		case 'aesthetic':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				costum('[❗] LOADING!', text, tescuk, ari)
				anu = await fetchJson(`https://api.zeks.xyz/api/estetikpic?apikey=GratisApi`, {method: 'get'})
		...
		case 'shota':
				...
				costum('[❗] LOADING!', text, tescuk, ari)
				try {
			    res = await fetchJson(`https://tobz-api.herokuapp.com/api/randomshota?apikey=Tobzzz17`)
		...
		case 'tinyurl':
                ara.updatePresence(from, Presence.composing) 
                data = await fetchJson(`https://tobz-api.herokuapp.com/api/tinyurl?url=${args[0]}&apikey=Tobzzz17`)
                hasil = `link : ${args[0]}\n\nOutput : ${data.result}`
                reply(hasil)
                await limitAdd(sender)
                break
        ...
        case 'memeindo':
				...
				gatauda = body.slice(8)
				costum('[❗] LOADING!', text, tescuk, ari)
				anu = await fetchJson(`https://api.zeks.xyz/api/memeindo?apikey=GratisApi`, {method: 'get'})
...
// CASE RANDOM
		...
		case 'next':
            ...
            default:
                  ...
                  reply(`Salken juga om, Aku Bot\nKetik .menu untuk melihat Menu Bot Botwea`)
                  }

					if (budy.includes(`salken`)) {
                  reply(`Salken juga om, Aku Bot Botwea\nKetik .menu untuk melihat Menu Bot Botwea`)
                  }
		 ...
		 if (budy.includes(`halo`)) {
                  reply(`Hallo Juga Om, Aku Bot Botwea\nKetik .menu untuk melihat Menu Bot Botwea`)
                  }
					if (budy.includes(`Halo`)) {
                  reply(`Hallo Juga Om, Aku Bot Botwea\nKetik .menu untuk melihat Menu Bot Botwea`)
                  }
					if (budy.includes(`hai`)) {
                  reply(`Hai Juga Om, Aku Bot Botwea\nKetik .menu untuk melihat Menu Bot Botwea`)
                  }
					if (budy.includes(`Hai`)) {
                  reply(`Hai Juga Om, Aku Bot Botwea\nKetik .menu untuk melihat Menu Bot Botwea`)
                  }
			...
			if (budy.includes(`assalamualaikum`)) {
                  reply(`Waalaikumsalam, Aku Bot Botwea\nKetik .menu untuk melihat Menu Bot Botwea`)
                  }
